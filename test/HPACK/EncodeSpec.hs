...
import Network.ByteOrder (withReadBuffer, withWriteBuffer)
import Network.HPACK
import Network.HPACK.Internal (decodeH, decodeS, encodeS)
import Test.Hspec
...
spec = do
    ...
    describe "encodeS" $ do
        it "round-trips a Huffman-coded string whose length needs four octets" $ do
            -- 'a' is a five-bit code, so these come to 5/8 of their length
            -- when Huffman-coded: 26416 is the first to need a four-octet
            -- length with a 7-bit prefix.  The fourth octet used to overwrite
            -- the first octet of the code.
            sequence_
                [ roundTripS n len
                | n <- [3, 5, 7]
                , len <- [100, 20000, 26415, 26416, 30000, 100000]
                ]
...
roundTripS :: Int -> Int -> Expectation
roundTripS n len = do
    let bs = C8.replicate len 'a'
        bufsiz = len * 4 + 64
    enc <- withWriteBuffer bufsiz $ \wbuf -> encodeS wbuf True id (`setBit` n) n bs
    gcbuf <- mallocPlainForeignPtrBytes bufsiz
    dec <-
        withReadBuffer enc $
            decodeS (.&. mask) (`testBit` n) n (decodeH gcbuf bufsiz)
    dec `shouldBe` bs
  where
    mask = (1 `shiftL` n) - 1
